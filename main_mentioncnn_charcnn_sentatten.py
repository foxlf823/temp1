...
from model.mentioncnn_charcnn_sentatten import CNNCNN_SentATTEN
...
def evaluate(data_loader, model, label_to_ix):
	...
	for j in range(num_it):
		...
		pred = model(mention_inputs, char_inputs, sentences)
...
if __name__ == "__main__":
	...
	model = CNNCNN_SentATTEN(vocab, len(meshlabel_to_ix), char_to_ix)
	...
	if opt.pretraining:
		...
		for epoch in range(opt.dict_iteration):
			...
			for i in range(dict_num_iter):
				...
				dict_batch_output = model(dict_inputs, dict_char_inputs, dict_sentences)
	...
	for epoch in range(opt.max_epoch):
		...
		for i in range(num_iter):
			...
			targets = utils.get_var(targets)
			batch_output = model(mention_inputs, char_inputs, sentences)
